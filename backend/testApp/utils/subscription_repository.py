...
class AbstractSubscriptionRepository(ABC):
    ...
    @abstractmethod
    async def create(self, subscriber_id: int, target_id: int, session: AsyncSession, status: SubscriptionStatus = SubscriptionStatus.ACTIVE) -> Subscription:
        raise NotImplementedError

    @abstractmethod
    async def get(self, subscriber_id: int, target_id: int, session: AsyncSession) -> Optional[Subscription]:
        raise NotImplementedError

    @abstractmethod
    async def delete(self, subscriber_id: int, target_id: int, session: AsyncSession) -> None:
        raise NotImplementedError

    @abstractmethod
    async def get_user_subscribers(self, user_id: int, session: AsyncSession) -> List[Subscription]:
        raise NotImplementedError

    @abstractmethod
    async def get_user_subscriptions(self, user_id: int, session: AsyncSession) -> List[Subscription]:
        raise NotImplementedError
...
class SQLAlchemySubscriptionRepository(AbstractSubscriptionRepository):
    ...
    async def get_user_subscribers_id(self, user_id: int, session: AsyncSession) -> List[int]:
        async with session.begin():
    ...
    async def update_status(self, subscriber_id: int, target_id: int, session: AsyncSession, new_status: SubscriptionStatus) -> Optional[Subscription]:
        async with session.begin():
    ...
    async def get_pending_subscriptions(self, user_id: int, session: AsyncSession) -> List[Subscription]:
        async with session.begin():
            stmt = select(self.model).where(
                (self.model.target_id == user_id) &
